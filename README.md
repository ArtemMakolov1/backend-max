...
Production хранит новые изображения в приватном S3-compatible хранилище.
Ключи доступны только backend и никогда не передаются frontend. Публичный URL
объекта не требуется: браузер продолжает получать файлы через защищённый
маршрут `/media`, где сервер проверяет сессию и владельца изображения.
...
`s3-nl.hostkey.com`: регион `nl` определяется автоматически. Для локальной разработки все S3
переменные можно оставить пустыми — тогда сохраняется файловое хранилище в
`MEDIA_DIR`. При запуске backend выполняет короткую запись, чтение и удаление
служебного объекта, поэтому ключу нужны права `ListBuckets` (только для
автоопределения), `HeadBucket`, `PutObject`, `GetObject` и `DeleteObject`.
Bucket должен оставаться закрытым и доступным только по ключам.
...
Незавершённые загрузки и файлы, на которые больше не ссылаются посты,
...
`MEDIA_CLEANUP_INTERVAL` и `MEDIA_CLEANUP_BATCH_SIZE`.
...
приватном bucket появляются только новые изображения.

...
7. Убедиться, что пользователь перед checkout повторно принял версии `terms` и `personal_data` от `2026-08-02`. Backend проверяет обе записи `user_consents` и возвращает `billing_legal_consent_required`, поэтому старой сессии потребуется повторный вход.

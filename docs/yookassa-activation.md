...
7. Убедиться, что действующие сессии повторно приняли версии `terms` и `personal_data` от `2026-07-22`. Автоматического re-consent для уже открытой сессии сейчас нет; до его продуктовой реализации live-платежи включать нельзя.
...
показать текст дословно и вернуть его версию при checkout. В журнал согласий
записываются ровно показанный текст, версия, ссылка, цена, валюта и версия
тарифа.

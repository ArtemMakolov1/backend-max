...
wordstat_api_key=$(env_value YANDEX_WORDSTAT_API_KEY)
wordstat_folder_id=$(env_value YANDEX_WORDSTAT_FOLDER_ID)
if [[ -n "$wordstat_api_key" && -z "$wordstat_folder_id" ]] ||
  [[ -z "$wordstat_api_key" && -n "$wordstat_folder_id" ]]; then
  echo "YANDEX_WORDSTAT_API_KEY and YANDEX_WORDSTAT_FOLDER_ID must be configured together" >&2
  exit 1
fi
if [[ -n "$wordstat_api_key" ]]; then
  [[ "$direct_parts" -eq 4 ]] || {
    echo "Yandex Wordstat requires complete Yandex Direct credentials" >&2
    exit 1
  }
  [[ "$wordstat_folder_id" =~ ^[a-z0-9]{1,50}$ ]] || {
    echo "YANDEX_WORDSTAT_FOLDER_ID must contain 1 to 50 lowercase letters or digits" >&2
    exit 1
  }
fi
...
expect_exact YANDEX_WORDSTAT_API_BASE_URL "https://searchapi.api.cloud.yandex.net"
